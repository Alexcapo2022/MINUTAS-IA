...
from app.controllers.donacion_controller import DonacionResponse, parse_donacion  # 👈 nuevo
...
@router.post("/donacion", response_model=DonacionResponse)
async def parse_donacion_endpoint(
    ...
    try:
        if file is None:
            raise HTTPException(
                status_code=400,
                detail="Debes adjuntar un archivo en 'file' (PDF o Word)."
            )
        return await parse_donacion(file)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
